...
1. Google·네이버 소셜 로그인으로 사전 등록한다. 현재 코드는 Google 로그인이며 네이버 연동은 예정이다.
...
4. 이벤트 시작 1~2시간 전에 등록자에게 이메일을 발송한다. AWS SES + EventBridge Scheduler 연동은 예정이며 정확한 시각은 미확정이다.
5. 다계정 추가 방지는 구현하지 않는다. 동일 계정·정규화 이메일 중복, 자기추천, 등록 후 추천인 변경만 차단한다.

현재 구현은 사전 등록, 양쪽 점수 반영, 내 점수 조회, 관리자 공동 순위 조회까지다.
상품 최종 선정·지급, 네이버 로그인, 메일 발송·스케줄 배포는 구현 완료로 표현하지 않는다.
